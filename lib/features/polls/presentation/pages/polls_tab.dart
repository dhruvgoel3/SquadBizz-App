...
/// Polls tab — shows all polls in a room with voting.
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.how_to_vote_rounded, size: 48, color: AppColors.primary),
          ),
...
/// Individual poll card with inline voting.
class _PollCard extends StatelessWidget {
  final Map<String, dynamic> poll;
  final String roomId;
  final bool isDark;

  const _PollCard({required this.poll, required this.roomId, required this.isDark});

  @override
  Widget build(BuildContext context) {
    final question = poll['question'] as String? ?? '';
    final options = (poll['options'] as List?)?.cast<Map<String, dynamic>>() ?? [];
    final createdAt = poll['created_at'] != null
        ? DateTime.tryParse(poll['created_at'].toString())
        : null;
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(
                      color: isSelected ? AppColors.primary : (isDark ? AppColors.borderDark : AppColors.dividerLight),
